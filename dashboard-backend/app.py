...
socketio = SocketIO(app, cors_allowed_origins="*", async_mode="gevent" if "gevent" in str(app.config) else "threading")
...
def calculate_session_summary():
    ...
    return {
        ...
        "total_alerts": len(alert_history),
        "total_blinks": last_record.get("total_blinks", 0),
        "total_yawns": last_record.get("total_yawns", 0),
        "session_duration_seconds": round(duration, 1)
    }
...
def export_csv():
    ...
    writer.writerow([
        ...
        "Total_Blinks", "Total_Yawns", "Gaze_Fixation_Time"
    ])
...
def reset_session():
    ...
    global session_start_time, latest_metrics
    session_start_time = time.time()
    latest_metrics = {}
